...
static zw::NavPara m_navPara = {{0, 0, 0}, {0, 0, 0}, false, false};
...
static sensor_msgs::PointCloud ValidaStartPath;  //存储优化后的路径点
...
int  local_Astarplan(int row,int col,int map_count,float submap_resolution,zw::CarPose localdesired);  //局部A*路径规划  暂时未用到
...
bool judge_localdesiredisoccd(zw::CarPose localdesired);   //判断局部目标点与局部起始点之间的直线上是否有障碍物
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber pose_sub = n.subscribe<geometry_msgs::PoseWithCovarianceStamped>("amcl_pose", 2, PoseReceived);  //订阅定位位姿
    ...
    ros::Publisher stopcar_pub = n.advertise<std_msgs::Int32>("main_Astar/Stopcar", 10, true);   //发布停车/发车指令

    cv::String map_path;
    if(argc>1)
      map_path = (cv::String)argv[1];
    else
      map_path= "/home/hulu/ros/zw/distributed_robot_sys/src/a_robot_platform/map/N301524mapnoimu.pgm";
    
	ptr = map_deal(&row,&col,map_path,&map_count); //处理全局.pgm地图，二值化并膨胀
    ROS_INFO("row=%d,cols=%d",row,col);
    ...
    while (ros::ok())
    {
       ...
       if(readsubmapfinish_flag)
       {
           if(ValidaStartPath.points.size()) 
           {
               ...
               static bool gloabplanfile=false;
               static zw::CarPose nextDest={0,0,0};
               ...
               if(iflag<size)
               {
                    ...
                    else 
                    {
                        ...
                        {
                            meetoccdcount++;
                            if(meetoccdcount>=4)    //若遇到障碍物的次数大于3次，则发送停车指令，并再次规划路径
                            {
                                ...
                                {
                                    gloabplanfile=true;   //路径规划失败，复位全局代价地图
                                    gloabcostmap_clear(gloabplanfile);
                                    A_starplan(ptr,row,col,map_count);
                                }
...
{
    static zw::CarPose lastCarPose={0,0,0};
    ...
    if(poseChangePow > poseChangePow2)
    {
        ...
        for (int i = 0; i < msg->info.height; i++)
            for (int j = 0; j < msg->info.width; j++)
            {
                maze[i][j] = msg->data[j + i * msg->info.width]; //origin down-left
                ...
                if(maze[i][j]==zw::kOccGrid)
                {
                    ...
                    if(readmapfinish_flag)    //将检测到的局部障碍物添加到全局代价地图
                    {
                        ...
                        maze_gloab_map[occd_map_row][occd_map_col]=zw::kOccGrid;
                    }
                }  
                else
                {
                    ...
                    {   
                        if(map_init_state[occd_map_row][occd_map_col]==zw::kFreeGrid)
                        {
                            ptr[occd_map_row][occd_map_col]=free_grid;
                            maze_gloab_map[occd_map_row][occd_map_col]=zw::kFreeGrid;
                        }
                        else ;
                    }
                }
...
void gloabcostmap_clear(bool gloabplanfile)
{
    ...
    geometry_msgs::Point32 submapstartxy;   //局部子图起始点相对于map坐标系的实际位置
    geometry_msgs::Point32 submapendxy;   //局部子图终点相对于map坐标系的实际位置
    ...
    for (int i = map_height-1; i>0; i--)
        for (int j = 0; j < map_width; j++)
        {
            if((i<=submapstart_row && i>=submapend_row) && (j>=submapstart_col && j<=submapend_col))   //该子图范围内的像素点属性不会被复位
            {
                ...
                {
                    if(map_init_state[i][j]==zw::kOccGrid)   //目的是为了将障碍物设置为BARRIER，自由空间设置为free_grid
                        ptr[i][j]=BARRIER;
                    else
                        ptr[i][j]=free_grid;
...
bool judge_localdesiredisoccd(zw::CarPose localdesired)
{
    ...
    zw::CarPose tempGoal{submap_fx, submap_fy, localdesired.h};
...
int  A_starplan(uchar ** ptr,int row,int col,int map_count)
{
    ...
    ptr[rows_current_x][cols_current_y]=STARTNODE;
       ...
       if(iflag==Validnewstartindex)
       {
          ...
          for(; iflag >= 0; iflag--)
          {
            ...
            if(iflag==0)
              ...
              worldPose.z = 0;                                                                 
            ValidaStartPath.points.push_back(worldPose);//该点云结构体存储的是规划好的相对于map坐标系的实际路径点坐标   此步将目标点放到队列的尾端
            printf("第%d个路径点:x为%6.2f,y为%6.2f,h为%6.2f \n",iflag,worldPose.x,worldPose.y,worldPose.z);
          }
       }
...
//局部路径规划,暂时未用到
int local_Astarplan(int row,int col,int map_count,float submap_resolution,zw::CarPose localdesired)
{
